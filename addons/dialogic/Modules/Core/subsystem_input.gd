...
func _release_references() -> void:
	if auto_skip != null and dialogic != null and dialogic.has_subsystem("History"):
		for connection: Dictionary in dialogic.History.get_signal_connection_list("visited_event"):
			dialogic.History.visited_event.disconnect(connection["callable"] as Callable)
		for connection: Dictionary in dialogic.History.get_signal_connection_list("unvisited_event"):
			dialogic.History.unvisited_event.disconnect(connection["callable"] as Callable)
	if auto_skip != null:
		for connection: Dictionary in auto_skip.get_signal_connection_list("toggled"):
			auto_skip.toggled.disconnect(connection["callable"] as Callable)
	auto_skip = null
	auto_advance = null
	manual_advance = null
